...
// ── Auth state helpers ────────────────────────────────────
function isLoggedIn() {
  // Aceita token JWT (conta real) ou flag legacy (modo demo / offline)
  return !!getToken() || localStorage.getItem('sq_loggedIn') === 'true';
}
...
// ── Demo mode ─────────────────────────────────────────────
function handleDemo() {
  console.log('[StudyQuest] Demo mode activated');
  setAuthUser({ email: 'demo@studyquest.app', password: '', demo: true, createdAt: Date.now() });
  launchApp();
}
...
async function launchApp() {
  ...
  // 1. Carrega localStorage SEMPRE como base (garante fallback offline)
  loadState();

  // 2. Se tiver token, tenta sobrepor com dados do backend
  if (getToken() && !isDemoMode()) {
    await loadUserDataFromAPI();

    // Token pode ter sido limpo (401) dentro de loadUserDataFromAPI
    if (!getToken()) {
      showAuthScreen();
  ...
  if (state.setup) {
    ...
    // Primeiro acesso → criação do herói
...
function logout() {
  clearAuth();
  clearToken(); // remove JWT do localStorage
  localStorage.removeItem('sq_loggedIn');
  const app = document.getElementById('app');
  if (app) { app.classList.remove('active'); app.style.display = 'none'; }
  showAuthScreen();
...
function initSettingsPage() {
  ...
  const cfgEmail = document.getElementById('cfg-account-email');
  if (cfgEmail && user && user.email) {
    cfgEmail.textContent = '📧 ' + user.email + (user.demo ? ' (demonstração)' : '');
  }
