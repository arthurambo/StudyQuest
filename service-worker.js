...
const CACHE_NAME = 'studyquest-cache-3';
...
self.addEventListener('widgetinstall', e => { e.waitUntil(_refreshWidget()); });
self.addEventListener('widgetresume',  e => { e.waitUntil(_refreshWidget()); });
self.addEventListener('widgetuninstall', () => {});
self.addEventListener('widgetclick', e => { e.waitUntil(self.clients.openWindow('./')); });

self.addEventListener('message', e => {
  if (e.data?.type === 'SQ_WIDGET_UPDATE') {
    _saveWidgetData(e.data.payload).then(() => _refreshWidget());
  }
});

async function _saveWidgetData(payload) {
...
async function _loadWidgetData() {
  try {
    const cache = await caches.open(DATA_CACHE);
...
function _buildWidgetMessage(streak) {
...
async function _refreshWidget() {
  ...
  if (!widget) return;
  const data = await _loadWidgetData();
  data.message = _buildWidgetMessage(data.streak || 0);
